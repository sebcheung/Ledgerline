...
RECONCILIATION_FINDING_TYPE = postgresql.ENUM(
    ...
    name="reconciliation_finding_type",
    create_type=False,
)
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("status", TRANSACTION_STATUS, nullable=False, server_default=sa.text("'posted'")),
        sa.Column(
            "reversal_of", sa.Uuid(as_uuid=True), sa.ForeignKey("transactions.id"), nullable=True
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "account_id", sa.Uuid(as_uuid=True), sa.ForeignKey("accounts.id"), nullable=False
        ),
    ...
    op.create_table(
        "account_balances",
        sa.Column(
            "account_id", sa.Uuid(as_uuid=True), sa.ForeignKey("accounts.id"), primary_key=True
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "event_id", sa.Uuid(as_uuid=True), sa.ForeignKey("outbox_events.id"), nullable=False
        ),
        sa.Column(
            "endpoint_id",
            sa.Uuid(as_uuid=True),
        ...
        sa.Column(
            "status", WEBHOOK_DELIVERY_STATUS, nullable=False, server_default=sa.text("'pending'")
        ),
        ...
        sa.UniqueConstraint("event_id", "endpoint_id", name="uq_webhook_deliveries_event_endpoint"),
    )
    op.create_index(
    ...
    op.create_table(
        ...
        sa.Column(
            "status", RECONCILIATION_RUN_STATUS, nullable=False, server_default=sa.text("'running'")
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "run_id", sa.Uuid(as_uuid=True), sa.ForeignKey("reconciliation_runs.id"), nullable=False
        ),
        sa.Column("finding_type", RECONCILIATION_FINDING_TYPE, nullable=False),
        sa.Column(
            "transaction_id", sa.Uuid(as_uuid=True), sa.ForeignKey("transactions.id"), nullable=True
        ),
        sa.Column(
            "settlement_line_id",
            sa.Uuid(as_uuid=True),
            sa.ForeignKey("settlement_lines.id"),
            nullable=True,
        ),
