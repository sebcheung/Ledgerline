...
- `worker/webhook_worker.py` (Phase 5) — the standalone process entry point: builds the `httpx.AsyncClient`, wires SIGTERM/SIGINT to a stop event, and runs `Dispatcher.run_forever`. Deliberately thin — all real logic is in `ledger.webhooks.dispatcher`, which is covered by the `ledger`-only coverage gate; `worker/` is not.
...
`dashboard/` and `worker/recon_scheduler.py` exist as empty package stubs so later phases don't require restructuring — they carry no logic yet. (`ledger/reconciliation/` is Phase 4, and `ledger/webhooks/dispatcher.py`/`signing.py` and `worker/webhook_worker.py` are Phase 5 — none of these are stubs anymore.)
