...
## `reclaim_count` is a separate column from `attempt_count`

**Chosen:** `webhook_deliveries.reclaim_count` (migration `0005`), incremented only by `Dispatcher.sweep_stale_claims`, independent of `attempt_count`.
**Rejected:** charging `sweep_stale_claims`'s reclaims against the existing `attempt_count` column instead of adding a new one.
**Why:** `sweep_stale_claims` deliberately never touches `attempt_count` when it reclaims a row -- a worker that crashed mid-POST made no observation of the receiver, so charging it an attempt would shrink the retry budget for a fault that was ours, not the receiver's (this is what makes delivery at-least-once rather than at-most-once, see `docs/ARCHITECTURE.md`). But that same reasoning means a worker that reliably crashes mid-POST (a bad deploy that dies right after the socket write, say) would leave `attempt_count` at 0 forever and be redelivered without limit. A separate counter, budgeted separately (`webhook_max_reclaims`, default 8, matching `webhook_max_attempts`), closes that gap without corrupting the semantics `attempt_count` already has: two different failure classes -- "we tried and the receiver told us something" versus "we don't know what happened, we just never heard back" -- get two different budgets, computed in one `CASE`-based `UPDATE` so the reclaim count and the dead-letter decision it gates can never observe each other's write half-applied.

## `run_forever` catches `Exception`, not `BaseException`

**Chosen:** `Dispatcher.run_forever`'s loop wraps `run_once()` in `except Exception`, logs, increments `db_errors_total{operation="dispatcher.run_once"}`, and backs off before the next cycle.
**Rejected:** `except BaseException`, or no `try/except` at all (the pre-fix state).
**Why:** `worker/webhook_worker.py` awaits `run_forever` directly at the top level with nothing above it to restart the loop -- before this fix, any exception (most importantly a `sqlalchemy.exc.OperationalError` from a Postgres backend that died mid-cycle) killed the worker process permanently, and every webhook delivery stayed `pending` forever until an operator noticed and manually restarted it. Catching `Exception` fixes that: a bad cycle is logged, counted, and backed off exactly like a normal poll interval before trying again. `BaseException` would also catch `asyncio.CancelledError`, which is raised into this coroutine specifically on task cancellation (SIGTERM-triggered graceful shutdown) -- swallowing that would turn a clean shutdown request into an infinite retry loop that never actually stops. `except Exception` is the one boundary that recovers from every fault this slice cares about while still letting shutdown work correctly.

## `asyncio.CancelledError` handling in `_deliver_one`: a regression guard, not a bug fix

**Chosen:** an explicit `except asyncio.CancelledError: ... raise` clause in `_deliver_one`, with a log line, ahead of the `except httpx.TimeoutException`/`except httpx.TransportError`/`except Exception` clauses below it.
**Why it's there:** `CancelledError` is a `BaseException` in Python 3.8+, so it was never actually caught by any of those `except Exception`-rooted clauses -- it already propagated correctly out of `deliver()` before this change. Adding the explicit clause changes no runtime behavior today.
**Why add it anyway:** the correct propagation was accidental in the sense that nothing pinned it down or asserted it -- a future "broaden this catch" refactor of `_deliver_one`'s error handling could silently start swallowing cancellation instead of propagating it, with no test to notice. The explicit clause plus `tests/chaos/test_worker_shutdown_cancellation.py` (which asserts cancellation propagates and the interrupted row lands exactly where `sweep_stale_claims` can recover it) turns an implicit, unverified correctness property into an explicit, tested one.
**Not oversold:** recorded here plainly as a regression guard and observability addition, matching the honest framing already in the chaos test's own docstring -- this entry should not be read as "Phase 8 fixed a cancellation bug," because there wasn't one.

## The webhook worker gets its own `/metrics` scrape target

**Chosen:** `worker/webhook_worker.py` calls `prometheus_client.start_http_server(settings.worker_metrics_port, registry=REGISTRY)` (port 9090 by default, `worker_metrics_enabled` kill switch) before entering `run_forever`, against the same `ledger.observability.metrics.REGISTRY` the app process uses. `fly.toml` gets a second `[[metrics]]` block (`processes = ["worker"]`, `port = 9090`); `docker-compose.yml`'s `webhook-worker` service publishes the same port for local scraping.
**Rejected (initially, mid-slice):** leaving `webhook_stale_claims_swept_total`, `webhook_delivery_attempts_total`, and `webhook_delivery_latency_seconds` as worker-process-only Counters/Histograms with no scrape path, on the theory that Phase 7's "the worker has no HTTP server" decision (above) still held and `outbox_lag_seconds` was signal enough.
**Why that rejection didn't survive contact with the alert rules:** it doesn't. `Counter`/`Histogram` state lives in each process's own Python interpreter; nothing shares it across `app` and `worker`. `GET /metrics` is served only by the `app` process (`fly.toml`'s original single `[[metrics]]` block), so `LedgerlineStaleClaimStorm` and `LedgerlineDeliveryFailureRatio` -- both written against worker-only Counters -- would read a flat, permanently-zero series against the real deployed topology: not "quiet," but blind. A "ship the alerts, document the gap" posture would have meant two of Phase 8's eight alerts were decorative from the day they shipped. Once that was measured (see `docs/CHAOS.md`'s note on how this was actually discovered, live, mid-slice), fixing it was small: `prometheus_client.start_http_server` is a few lines, needs no ASGI app, and Fly's private-network Prometheus scraper doesn't need an `[http_service]`/`[[services]]` block to reach a second `[[metrics]]` port on the same machine.
**Why not convert these to DB-refreshed Gauges instead** (the pattern `webhook_dlq_depth`/`outbox_lag_seconds` already use): `webhook_delivery_latency_seconds` is a Histogram over a distribution of individual delivery latencies -- there is no table row that reconstructs a quantile after the fact, only a second, parallel bookkeeping table that would have to be invented and maintained just to feed a Gauge. A second scrape target is less new surface than a second persistence mechanism for data Prometheus already knows how to store as a histogram natively.
**Consequence, accepted:** the worker now has a second thing that can go wrong operationally (a bound port, a second scrape target to keep healthy) in exchange for `LedgerlineStaleClaimStorm`/`LedgerlineDeliveryFailureRatio` actually being able to fire. `outbox_lag_seconds` remains the cross-process liveness signal for "is the worker still making progress at all" -- the new scrape target is for "what is it doing," which is a different and more detailed question.

## Alert rules ship without a deployed Prometheus/Alertmanager

**Chosen:** `ops/alerts.yml` and `ops/alerts_test.yml`, validated in CI by `promtool check rules` (structural/PromQL correctness) and `promtool test rules` (behavioral: do the more false-positive-prone rules actually fire/not-fire where expected), both run via the pinned `prom/prometheus:v3.5.0` Docker image -- no Prometheus server, Alertmanager, or scrape config is deployed anywhere.
**Rejected:** standing up an actual Prometheus instance (on Fly or elsewhere) scraping `/metrics` and evaluating these rules live, as part of this slice.
**Why:** SPEC.md §12 Phase 8 asks for alert rules and a runbook, not a monitoring platform deployment -- the same "deploy-ready, not deployed" posture Phase 7 took with `fly.toml`/CD (`docs/DECISIONS.md` Phase 7). Static validation via `promtool` is exactly what's achievable and meaningful on a CI runner (the same reasoning that keeps the Locust load test out of CI, above): it proves the rules are syntactically valid PromQL and that the ones with dedicated fixture data in `ops/alerts_test.yml` fire and clear at the thresholds they claim to, without needing a live time-series backend or a real incident to exercise them against.
**Consequence, accepted:** these alerts do not page anyone today. Deploying an actual Prometheus that scrapes `https://<app>.fly.dev/metrics` and an Alertmanager to route `ops/alerts.yml`'s `severity` labels somewhere is deliberately left to whoever operates a real deployment, the same way `docs/DEPLOY.md`'s one-time Fly setup is left to a human rather than performed by this repository's automation.

## `pg_terminate_backend` for real backend kills in `tests/chaos/`, alongside `tests/faults/`'s monkeypatched crashes

**Chosen:** `tests/chaos/_pg_kill.py` (`backend_pid`/`kill_backend`) terminates a real Postgres backend serving the connection under test, from an independent connection, at a precise injection point (right after `post_transaction`'s real `FOR UPDATE` lock is acquired, or right after `claim_key`'s commit) -- used by `test_db_loss_mid_posting.py`, `test_db_loss_mid_idempotent_request.py`, `test_worker_db_loss_survives.py`, and `test_metrics_survive_db_loss.py`.
**Rejected:** relying solely on `tests/faults/test_idempotency_crash.py`'s established `SimulatedCrash`/`BaseException`-injection technique for this phase's new DB-loss coverage.
**Why both techniques stay, doing different jobs:** `SimulatedCrash` is deterministic and instant, and it exercises exactly one thing well -- what happens when a code path is interrupted at a specific line with no cleanup running at all, the way a SIGKILL would look from the process's own point of view. It proves nothing about what a *real* dead connection does to SQLAlchemy's session state, connection pool, or in-flight query. `pg_terminate_backend` is real infrastructure chaos: nothing about `post_transaction`'s or `IdempotentRequest.run`'s control flow is altered, only the backend serving the connection is pulled out from under it, so what's actually being proven is that Postgres's own transactional rollback-on-disconnect -- not any application-level `try`/`except` -- is what keeps a killed mid-write from leaving a half-posted transaction behind. `test_db_loss_mid_idempotent_request.py`'s finding (the claim is cleanly released via `AsyncSession.rollback()`'s handling of an unusable connection, checking out a fresh one from the pool for `release_key`, rather than left stuck for the TTL) is a *stronger* result than the equivalent simulated-crash test in `tests/faults/`, and that difference is only visible by testing the real thing.

## `refresh_db_gauges` degrades to stale-but-present values on a DB error

**Chosen:** `refresh_db_gauges` wraps its entire body in one `try/except Exception`, logs, increments `metrics_db_refresh_failures_total`, and does nothing else on failure -- every Gauge it maintains keeps whatever value it last successfully `.set()`.
**Rejected:** letting the exception propagate (so `GET /metrics` itself returns 500 on a DB outage); catching it and setting every gauge to some fallback sentinel value (e.g. `-1` or `NaN`); running a simpler fallback query on failure.
**Why not raise:** `/metrics` is the one endpoint an operator needs most during an outage -- SPEC.md §9's whole point is observability, and an observability endpoint that goes down exactly when the system is unhealthy is a defect, not a correct failure mode. This was, in fact, the concrete bug this slice's metrics work fixed: before it, a DB outage during a `/metrics` scrape produced a 500 with no gauges at all, rather than a 200 with stale-but-present ones.
**Why not a fallback sentinel:** a `-1` or `NaN` would need every alert expression referencing these gauges to special-case "the refresh is currently broken" as a distinct value from "the real count is legitimately low" -- `LedgerlineMetricsStale` already exists specifically so an operator (or an alert) can independently know the gauges might be stale, via `metrics_db_refresh_failures_total`, without polluting the gauges' own value space with an out-of-band signal.
**Why "do nothing" is sufficient, not lazy:** `prometheus_client` Gauges already retain their last `.set()` value when `.set()` is not called again -- there is no "unset" state to fall back from. The last successfully-scraped value is a strictly better answer than any fallback query could produce (a fallback query would just be racing the same DB outage that already failed once) and is exactly what `LedgerlineMetricsStale`'s own alert description already tells an operator to distrust once it's firing.
